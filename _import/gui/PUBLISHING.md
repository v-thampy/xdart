...
Since xdart depends on `ssrl_xrd_tools>=0.41.0`, you must publish ssrl_xrd_tools first.
...
- `ssrl_xrd_tools-0.41.0.tar.gz` (source distribution)
- `ssrl_xrd_tools-0.41.0-py3-none-any.whl` (wheel)
...
pip install dist/ssrl_xrd_tools-0.41.0-py3-none-any.whl
...
pip install dist/xdart-0.40.0-py3-none-any.whl
...
source:
  ...
  # Get sha256 from: pip hash dist/ssrl_xrd_tools-0.41.0.tar.gz
...
```bash
pip hash dist/ssrl_xrd_tools-0.41.0.tar.gz
# Or after uploading to PyPI:
curl -s https://pypi.org/pypi/ssrl_xrd_tools/0.41.0/json | python -c "import sys,json; d=json.load(sys.stdin); print([f['digests']['sha256'] for f in d['urls'] if f['filename'].endswith('.tar.gz')][0])"
```

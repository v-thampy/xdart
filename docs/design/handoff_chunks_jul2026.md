...
| BW-A1b | Blocker wave lane A LOG-1 run-start cap logging | S | low | live stress logs now print `reduction workers: N` and `heavy window: N frames` whenever the streaming run/session start path is reused, not only at lazy construction | MEM-2/MEM-3 | fixed-unverified (this commit); gate: MEM-2 wiring/reuse log regression |
| BW-A2 | Blocker wave lane A append integrity: BL-1 + BL-2 | S | high | Append target degraded-load aborts loudly before any write and preserves existing `.nxs` bytes; Append-loaded rows are marked persisted so the first batch flush appends instead of atomic full-rewriting prior data | CF/Append integrity | fixed-unverified (this commit); gates: degraded-load drill, append snapshot, Qt sink first-flush |
| BW-A3 | Blocker wave lane A S-2 append config signature guard | S | med | Append initialize reads the target file's stored processing config under the run lock and aborts before any frame processing when it differs from the current panel config, even for live/cold runs with empty `img_file`; stored and current signatures now share canonical bool/default/range/float normalization, and same-config Append passes | CF-1/CF-2/CF-3 | fixed-unverified (`947750b6` + bounce fix + modal-unify); gates: Append initialize mismatch/same-config/Overwrite, Controls V2 same/unrelated-target clickable-modal flow, full GI spine 71/71 |
| BW-A4 | Blocker wave S-8/S-11/S-12/S-9/S-10 sweep | M | med | parsed series first-frame filter, fail-loud zero-discovery summary, live Overall aggregates use the writer scan tail including the NeXus live worker handoff, invalidate only on flush signature, GUI store consumes the frame-precise heavy window, and all five late writer sites lock before pausing the H5 pool | BW-A3 | fixed-unverified (`6f0b726f` + R4 NeXus active-scan follow-up); gates: append skip, MEM-2 wiring, pause/resume lock order, aggregation wiring, NeXus active-scan handoff, scan aggregate, full GI spine |
| BW-A5 | Final pretag robustness batch: S-13 + S-15 + CP-v2 wins | S | med | close-time load/stitch worker cleanup no longer destroys running QThreads after bounded waits; image contrast level reuse is scoped by scan/display generation and cleared on blank; Controls V2 locks action/more controls and advanced dialog during runs, refreshes source/metadata caches on filesystem mtime, clamps point counts, preserves browse-cancel paths, and warns once on refresh failure | BW-A4 | fixed-unverified (this commit); gates: S-13/S-15/CP-v2 focused tests, full Controls V2, live-refresh, GUI modes, full GI spine 71/71 |
...
| OV-7 | Pinned slice cuts: slice projection identity + Pin action | M | med | texture workflow can overlay multiple χ/q cuts from the same frame; live cut mutates in place while pinned cuts survive norm/BG/unit rebuilds through recipes; live current cut is gray/lightweight, does not consume palette colors, previews the next pin offset slot, and Pin freezes it in place | OV-6, H30 | fixed-unverified; verify Session-1 texture-cut workflow incl. OV-7c no-overlap/no-jump |

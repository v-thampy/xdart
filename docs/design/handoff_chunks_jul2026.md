...
| OV-7 | Pinned slice cuts: slice projection identity + Pin action | M | med | texture workflow can overlay multiple χ/q cuts from the same frame; live cut mutates in place while pinned cuts survive norm/BG/unit rebuilds through recipes; live current cut is gray/lightweight and does not consume palette colors | OV-6, H30 | fixed-unverified; verify Session-1 texture-cut workflow |

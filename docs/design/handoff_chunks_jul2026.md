...
| BW-A3 | Blocker wave lane A S-2 append config signature guard | S | med | Append initialize reads the target file's stored processing config under the run lock and aborts before any frame processing when it differs from the current panel config, even for live/cold runs with empty `img_file`; Controls V2 now blocks same-target loaded-scan mismatches from the loaded scan's stored config | CF-1/CF-2/CF-3 | fixed-unverified (this commit); gates: Append initialize mismatch + Controls V2 same/unrelated-target blockers |

    ...
    for segments in (1, 2):
        ...
        for index in range(segments):
            ...
            with h5py.File(data_path, "w") as handle:
                ...
                if segments == 2 and index == 1:
                    payload = root / "eiger_selected_dependency.bin"; node = group.create_dataset("data", shape=data.shape, dtype=data.dtype, external=[(str(payload), 0, h5py.h5f.UNLIMITED)]); node[...] = data; dependencies.append(SourceFileState.capture(payload))
                else: group.create_dataset("data", data=data, chunks=(1, *shape))
...
def test_plan_preparation_recipe_roundtrip_share_one_runner_and_writer(tmp_path, monkeypatch):
    ...
    plan_keys = {"api_version", "target", "entry", "expected_target_snapshot", "dimension", "labels", "detector_shape", "native_dtype", "selected_plan", "requested_shared_science", "gi_bootstrap_incidence", "session_policy", "rollback_policy", "science_identity", "operation_identity"}
    ...
    facts = (named_plan.target, named_plan.entry, named_plan.dimension, named_plan.labels, named_plan.detector_shape, named_plan.native_dtype, module._plain(named_plan.selected_plan), module._plain(named_plan.requested_shared_science)); first = module._make_plan(*facts, named_plan.gi_bootstrap_incidence, named_plan.session_policy, snapshot=named_plan.expected_target_snapshot); second = module._make_plan(*facts, .3, named_plan.session_policy, snapshot=named_plan.expected_target_snapshot)

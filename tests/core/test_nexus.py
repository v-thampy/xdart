...
class TestWriteNexusResult1D:
    ...
    def test_nxdata_class(self, tmp_path, result_1d):
        ...
        with h5py.File(p, "r") as f:
            ...
            assert list(grp.attrs["axes"]) == ["frame_index", "axis_1"]

    def test_q_axis_values(self, tmp_path, result_1d):
        p = write_nexus(tmp_path / "a.nexus", results_1d={0: result_1d})
        with h5py.File(p, "r") as f:
            np.testing.assert_allclose(
                f["entry/integrated_1d/axis_1"][()], result_1d.radial, rtol=1e-6,
            )
    ...
    def test_unit_on_q_axis(self, tmp_path, result_1d):
        p = write_nexus(tmp_path / "a.nexus", results_1d={0: result_1d})
        with h5py.File(p, "r") as f:
            assert f["entry/integrated_1d/axis_1"].attrs["units"] == "q_A^-1"
...
class TestWriteNexusResult2D:
    ...
    def test_nxdata_class(self, tmp_path, result_2d):
        ...
        with h5py.File(p, "r") as f:
            ...
            assert grp.attrs["NX_class"] == "NXdata"
            assert grp.attrs["signal"] == "intensity"
            assert list(grp.attrs["axes"]) == ["frame_index", "axis_2", "axis_1"]

    def test_axes_values(self, tmp_path, result_2d):
        p = write_nexus(tmp_path / "a.nexus", results_2d={0: result_2d})
        with h5py.File(p, "r") as f:
            np.testing.assert_allclose(
                f["entry/integrated_2d/axis_1"][()], result_2d.radial, rtol=1e-6,
            )
            np.testing.assert_allclose(
                f["entry/integrated_2d/axis_2"][()], result_2d.azimuthal, rtol=1e-6,
            )
    ...
    def test_unit_on_q_axis(self, tmp_path, result_2d):
        p = write_nexus(tmp_path / "a.nexus", results_2d={0: result_2d})
        with h5py.File(p, "r") as f:
            assert f["entry/integrated_2d/axis_1"].attrs["units"] == "q_A^-1"
...
class TestRoundtrip:
    def test_1d_full_roundtrip(self, tmp_path, sample_metadata, result_1d):
        ...
        with h5py.File(p, "r") as f:
            ...
            np.testing.assert_allclose(g["axis_1"][()], result_1d.radial, rtol=1e-6)
            for row in (0, 1):
            ...
            assert g["axis_1"].attrs["units"] == "q_A^-1"
    ...
    def test_2d_full_roundtrip(self, tmp_path, result_2d):
        ...
        with h5py.File(p, "r") as f:
            ...
            np.testing.assert_allclose(g["axis_1"][()], result_2d.radial, rtol=1e-6)
            np.testing.assert_allclose(g["axis_2"][()], result_2d.azimuthal, rtol=1e-6)
    ...
    def test_incremental_same_as_batch(self, tmp_path, result_1d, result_2d):
        ...
        with h5py.File(p_batch, "r") as fb, h5py.File(p_incr, "r") as fi:
            for key in ("integrated_1d/axis_1", "integrated_1d/intensity",
                        "integrated_2d/axis_1", "integrated_2d/axis_2", "integrated_2d/intensity"):

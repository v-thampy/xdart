...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("axis", "/entry/integrated_1d/axis_1", "current xdart"),
...
def test_frame_view_refuses_malformed_or_oversized_nodes_before_getitem(
    ...
    with h5py.File(path, "w") as handle:
        ...
        if case == "inventory":
            ...
            group = entry["integrated_1d"]
            del group["axis_1"]
            group.create_dataset("axis_1", shape=(1_000_001,), dtype=np.float64, chunks=(64,))
        elif case == "scan_data":
...
def test_frame_view_bounds_source_path_and_text_attributes_without_getitem(
    ...
    with h5py.File(path, "r+") as handle:
        ...
        handle["entry/integrated_1d/axis_1"].attrs["units"] = "u" * 257
    ...
    with h5py.File(path, "w") as handle:
        if node == "entry":
            ...
            if node == "integrated_1d":
                ...
                del entry["integrated_1d/axis_1"]
                entry["integrated_1d"]["axis_1"] = h5py.ExternalLink(
                    str(foreign), "/entry/integrated_1d/axis_1",
                )

    ...
    *,
    ...
    with h5py.File(path, "w") as handle:
        ...
        if results:
            ...
            group.attrs["axes"] = ("frame_index", "axis_2", "axis_1")
            ...
            group.create_dataset("axis_2", data=np.arange(3, dtype=np.float32))
            group.create_dataset("axis_1", data=np.arange(4, dtype=np.float32))
        if detector:
            entry.create_dataset(
                "instrument/detector/data",
                data=np.ones((2, 4, 5), dtype=np.uint16),
...
@pytest.mark.parametrize(
    "dataset_name", ("frame_index", "axis_1", "axis_2", "intensity", "sigma"),
)

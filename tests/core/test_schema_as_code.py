...
def test_row_aligned_and_axis_declarations():
    ...
    assert SCHEMA.groups["integrated_1d"].axes == ("axis_1",)
    assert SCHEMA.groups["integrated_2d"].axes == ("axis_1", "axis_2")
...
def test_drop_integrated_rows_slices_only_schema_row_set(tmp_path):
    ...
    with h5py.File(p, "w") as f:
        ...
        g.create_dataset("axis_1", data=np.linspace(0.0, 1.0, 5))
        ...
        assert g["axis_1"].shape == (5,)              # shared axis untouched
...
def test_dataset_spec_facts_are_frozen():
    ...
    assert set(g1) == {"intensity", "axis_1", "frame_index", "sigma"}
    assert set(g2) == {"intensity", "axis_1", "axis_2", "frame_index", "sigma"}
    for g in (g1, g2):
    ...
    assert g1["axis_1"].units_from == "radial_unit"
    assert g2["axis_2"].units_from == "azimuthal_unit"

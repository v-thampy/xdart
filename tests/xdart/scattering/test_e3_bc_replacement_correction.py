...
class _AdmissionPort:
    """Exact active/queued fake used only for admission rollback."""
    ...
    def __init__(self, base) -> None:
        ...
        self.cancelled = base.cancelled

    def begin(self, request):
        if self.active is None:
            self.active = request
        elif self.queued is None:
            self.queued = request
        else:
            raise RuntimeError("one queued replacement")
    ...
    def owns_request(self, request):
        return request is self.active or request is self.queued
    ...
    def consume(self, outcome):
        ...
        self.queued = None
        value, self.context = self.context, None
        return value
    ...
    def release_context(self, context):
        return self._base.release_context(context)

    def close(self, expected=None):
        return BrowseCleanupReceipt(expected, CleanupStatus.CLEANED)


def test_third_request_rejection_preserves_exact_queued_d_hold() -> None:
    controller, lifecycle, executor, base, acquisition = _running_controller()
    port = _AdmissionPort(base)
    controller._browse_loader = port
    controller.pause()
    request_b = controller.begin_browse("/processed/browse.b.nxs")
    _, browse_b = _browse(
        request_b.token,
        request_b.load_generation,
        scan_key="browse.b",
        request=request_b,
    )
    port.context = browse_b
    port.outcome = BrowseLoadOutcome(request_b, BrowseLoadStatus.READY)
    assert controller.poll_browse() is port.outcome
    port.active = None
    selection_b = controller.selection
    navigation_b = controller.navigation

    request_c = controller.begin_browse("/processed/browse.c.nxs")
    request_d = controller.begin_browse("/processed/browse.d.nxs")
    pending_d = controller._runtime._pending_replacement
    generation = controller._load_generation

    with pytest.raises(RuntimeError, match="queued replacement"):
        controller.begin_browse("/processed/browse.e.nxs")

    assert controller._load_generation == generation
    assert controller._browse_request is request_d
    assert controller._runtime._pending_replacement is pending_d
    assert pending_d.request is request_d
    assert pending_d.selection is selection_b
    assert pending_d.navigation is navigation_b
    assert port.active is request_c
    assert port.queued is request_d
    assert lifecycle.phase.value == "paused"
    assert executor.identity is controller.run_identity
    assert controller.retained_contexts == (acquisition,)

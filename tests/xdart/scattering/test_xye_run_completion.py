"""A completed real XYE-only Run opens its published curves, not its raw input."""
...
def test_completed_xye_run_opens_exact_generated_files(
    tmp_path, monkeypatch, source_kind, batch, unit, prefix,
):
    app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
    images = np.stack([np.full((195, 487), value, np.uint16)
    ...
    output.mkdir()
    stale = output / ("raw" if source_kind == "tiff" else "raw_master")
    stale.mkdir()
    ...
    try:
        ...
        assert terminal.completed == terminal.total == 3
        ...
        assert set(context.paths) == {str(path) for path in generated}
        assert context.current_path in context.paths
        navigation = page._context_controller.navigation
        assert len(navigation.frames) == len(navigation.selected) == 3
        scientific = page._last_scientific_projection
        assert len(scientific.traces) == 3
        ...
        assert len(page._shell.scientific.curve.getPlotItem().listDataItems()) == 3
    finally:

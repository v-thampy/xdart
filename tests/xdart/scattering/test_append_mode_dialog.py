...
@pytest.mark.parametrize("initial_gi,reopen,source_kind", (
    (False, False, "tiff"), (True, False, "tiff"), (False, True, "tiff"),
    (False, False, "eiger"),
))
def test_stopped_mode_change_requires_cancel_or_overwrite(tmp_path, monkeypatch, initial_gi, reopen, source_kind):
    app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
    if source_kind == "eiger":
        raw = tmp_path / "scan_master.h5"
        _write_eiger(raw, tmp_path / "scan_data_000001.h5", 16)
        expected_labels = np.arange(16)
    else:
        ...
        expected_labels = np.arange(1, 17)
    ...
    if source_kind == "eiger":
        intent.gi.incidence_motor = "th"  # Stale motor from the previous TIFF scan.

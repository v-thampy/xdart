...
def test_append_run_start_seeds_processed_frame_browser_without_render():
    ...
    thread = admitted_worker(
        SimpleNamespace(_append_skip_snapshot=lambda name: {1, 2, 3}),
        output_mode="Append")
    host = SimpleNamespace(
        scan=scan,
        h5viewer=viewer,
...
def test_live_current_slice_trace_is_gray_lightweight_and_skips_palette():
    ...
    # OV-7b/7c: the "· current" cut is a gray DASHED LINE ONLY (no marker) at
    # ~75% opacity, and it SKIPS the palette (so calls[2] gets the 2nd color).
    assert _rgb(calls[1]["pen"]) in {(95, 95, 95), (170, 170, 170)}
    assert calls[1]["pen"].style() == QtCore.Qt.DashLine
    assert calls[1]["pen"].widthF() == pytest.approx(1.5)
    assert calls[1]["pen"].color().alpha() == 191          # ~75% opacity
...
def test_reintegrate_excludes_publication_dropped_frame_from_finalize(monkeypatch):
    """A complete reintegrate where the writer publication-dropped one frame
    swaps successfully, excluding the dropped frame from the shadow's expected
    coverage (Finding 1).  The shadow is NOT dropped — the prior all-or-nothing
    rollback over one all-dummy GI 2D row is gone."""
    import threading
    from types import MethodType, SimpleNamespace
    ...
    def _shadow_write(idxs, **k):
        ...
        # The writer's publication gate dropped frame 2 (e.g. an all-dummy GI
        # 2D row); surface it so the loop subtracts it from `expected`.
        return {2}
    ...
    assert writes == [[0, 1, 2, 3, 4]]
    # Frame 2 is excluded from the expected coverage handed to finalize, so the
    # swap succeeds instead of rolling back.
    assert finalizes == [{
        "do_2d": False,

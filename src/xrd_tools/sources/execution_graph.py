...
def validate_source_aliases(
    ...
    *,
    ...
    if resolve_alias is None:
        resolve_alias = lambda path: str(Path(path).resolve(strict=True))
    if capture_target is None:
        capture_target = lambda path: SourceFileState.capture(Path(path))
    if check_candidate_owner and candidate_owner_id is None:
        from xrd_tools.sources.adapters import candidate_owner
        candidate_owner_id = lambda path: (
            None if (owner := candidate_owner(Path(path))) is None else owner.id
        )

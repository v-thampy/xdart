...
class ControlPanelRenderState:
    ...
    ``profile`` is the design-level status/gating description and
    ``bound_controls`` is the value-only form snapshot supplied by the owning
    page.  The renderer neither reads widgets nor owns editable run state.
    """
    ...
    actions = {
        SectionId.SOURCE: (
            ControlActionSpec(
                ...
                enabled=unlocked,
                reason=(
                    "Opens the current source browser."
                    if unlocked
                    else "Controls are locked during the active run."
                ),
            ),
